...
import {
  ...
  MessageSquare, Send, Star, Phone, CheckCircle2,
} from 'lucide-react';
...
const CONTACT_INITIAL: ContactFormState = { name: '', email: '', phone: '', type: 'query', message: '' };

const CONTACT_FIELD =
  'w-full bg-white border border-stone-200 rounded-xl px-4 py-3 text-sm text-stone-800 placeholder-stone-400 focus:outline-none focus:border-[var(--brand-500)] focus:ring-1 focus:ring-[var(--brand-500)]/30 transition-all duration-200';

// ─── Tab types ────────────────────────────────────────────────────────────────

type Tab = 'orders' | 'addresses' | 'settings' | 'contact';
...
const ProfilePage: React.FC = () => {
  ...
  // Honour ?tab=contact (or any valid tab) from URL
  const tabParam = searchParams.get('tab') as Tab | null;
  const initialTab: Tab = tabParam && ['orders', 'addresses', 'settings', 'contact'].includes(tabParam)
    ? tabParam
    : 'orders';
  ...
  // Contact tab state
  const [contactForm, setContactForm]     = useState<ContactFormState>(CONTACT_INITIAL);
  ...
  const handleContactSubmit = async (e: React.FormEvent) => {
    ...
    const res = await api.submitQuery(contactForm);
            ...
            {activeTab === 'contact' && (
              <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
                ...
                <div className="px-6 py-5 border-b border-stone-100 flex items-center gap-3">
                  <div className="h-9 w-9 rounded-full bg-[var(--brand-600)]/10 flex items-center justify-center shrink-0">
                    <MessageSquare className="h-4 w-4 text-[var(--brand-600)]" />
                  </div>
                  <div>
                    <h2 className="font-semibold text-gray-900">Get in Touch</h2>
                    <p className="text-xs text-stone-400 mt-0.5">We'll reply within 24 hours</p>
                  </div>
                </div>
                ...
                <div className="p-6">
                  {contactSent ? (
                    <div className="flex flex-col items-center justify-center py-12 text-center gap-4">
                      <div className="h-16 w-16 rounded-full bg-[var(--brand-600)]/10 flex items-center justify-center">
                      ...
                      <button
                        onClick={() => { setContactSent(false); setContactForm(CONTACT_INITIAL); }}
                        className="text-sm text-[var(--brand-600)] hover:underline font-medium"
                      >
                    ...
                    <form onSubmit={handleContactSubmit} className="space-y-5 max-w-lg">
                      ...
                      <div className="flex gap-2 p-1 bg-stone-100 rounded-xl">
                        {(['review', 'query'] as const).map((t) => (
                          <button
                            key={t}
                            type="button"
                            onClick={() => setContactForm((prev) => ({ ...prev, type: t }))}
                      ...
                      {/* Phone */}
                      <div className="space-y-1.5">
                        <label className="block text-xs font-medium text-stone-500 tracking-wide uppercase">Phone Number</label>
                      ...
                      <div className="space-y-1.5">
                        ...
                        <textarea
                          ...
                          rows={5}
                          placeholder={contactForm.type === 'review' ? 'Share your experience with us…' : 'Describe your query in detail…'}
                          required
                          className={`${CONTACT_FIELD} resize-none`}
                        />
                      </div>
                      ...
                      <button
                        ...
                        className="w-full sm:w-auto flex items-center justify-center gap-2 bg-[var(--brand-600)] hover:bg-[var(--brand-700)] disabled:opacity-60 disabled:cursor-not-allowed text-white text-sm font-semibold px-8 py-3 rounded-xl transition-all duration-200 shadow-md hover:shadow-lg"
                      >
                        {contactSubmitting ? (
                          <><div className="h-4 w-4 border-2 border-white/40 border-t-white rounded-full animate-spin" />Sending…</>
                        ) : (
                          <><Send className="h-4 w-4" />{contactForm.type === 'review' ? 'Submit Review' : 'Send Message'}</>
                        )}
                      </button>
                    </form>
                  )}
...
// Wrap in Suspense so useSearchParams() doesn't break SSR in Next.js 14
